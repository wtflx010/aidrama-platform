"""批量资产生成封面编排任务：轮询子任务进度，汇总成功/失败。

与 batch_keyframes 同构：父任务通过 provider_task_id（JSON 子任务 ID 列表）
跟踪各资产封面生成进度，全部结束后汇总结果。
"""
import json
import time
...
BATCH_TIMEOUT = 3600  # 60 分钟（图片生成慢，放宽超时）


@celery_app.task(name="batch_asset_covers", bind=True)
def batch_asset_covers(self, task_id: str):
    db = SessionLocal()
    try:
        ...
        failed_ids = [
            ...
            if (db.get(Task, sid) or Task(status=TaskStatus.failed)).status == TaskStatus.failed
        ]
        if failed_ids:
            update_task(
                db, task_id, status=TaskStatus.failed,
                error=f"{len(failed_ids)}/{len(sub_ids)} 个资产封面失败：{failed_ids[:5]}",
                finished_at=now(),
            )
        else:
