...
BATCH_TIMEOUT = 1800  # 30 分钟


@celery_app.task(name="batch_keyframes", bind=True)
def batch_keyframes(self, task_id: str):
    db = SessionLocal()
    try:
        task = db.get(Task, task_id)
        if task is None:
            return
        # 子任务 ID 存在 provider_task_id（JSON 字符串）
        ...
        if failed_ids:
            update_task(
                db, task_id, status=TaskStatus.failed,
                error=f"{len(failed_ids)}/{len(sub_ids)} 个关键帧失败：{failed_ids[:5]}",
                finished_at=now(),
            )
        else:
